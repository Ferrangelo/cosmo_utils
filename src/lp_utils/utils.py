...
def write_lp_setup_results_json(
    ...
    *,
    ...
    """Write LP aggregation results to a JSON file in the reference format.

    Parameters
    ----------
    output_path : str | Path
        Destination filename for the JSON file.
    agg : AggregateResults-like
        Aggregated statistics object from lp_analysis.aggregate_lp_statistics.
        Must expose attributes: n_success, chi2_reduced_mean, mean_sigma_lp,
        mean_lp, std_lp, ks_lp_pvalue, ks_chi2_pvalue, lp_values, sigma_lp_values.
    results : Sequence[RealizationResult-like]
        List of per-realization results used to compute Ntot and failures.
    LP_true : float
        Fiducial Linear Point value used to compute relative metrics.
    extra_fields : dict, optional
        Additional key/value pairs to merge into the output JSON.
    ...
    Ntot = len(results)
    CountNoPeakSave = Ntot - int(getattr(agg, "n_success", 0))
    failures_perc = 100.0 * CountNoPeakSave / Ntot if Ntot else float("nan")

    mean_error = (
        float(agg.mean_sigma_lp)
        if getattr(agg, "mean_sigma_lp", None) is not None
        else float("nan")
    )
    mean_rela_error = (
        mean_error / float(LP_true)
        if (mean_error and not np.isnan(mean_error))
        else float("nan")
    )
    extraction_bias = (
        (float(agg.mean_lp) - float(LP_true)) / mean_error
        if (mean_error and not np.isnan(mean_error))
        else float("nan")
    )

    LPdistrSave = [float(x) for x in getattr(agg, "lp_values", [])]
    LPdistrErrSave = [float(x) for x in getattr(agg, "sigma_lp_values", [])]

    out_dict = {
        "Ntot": int(Ntot),
        "CountNoPeakSave": int(CountNoPeakSave),
        "failures_perc": float(failures_perc),
        "mean_chi2/DOF": float(
            getattr(agg, "chi2_reduced_mean", float("nan")) or float("nan")
        ),
        "mean_error": float(mean_error),
        "mean_rela_error": float(mean_rela_error),
        "LP_fid": float(LP_true),
        "LP_mean": float(getattr(agg, "mean_lp", float("nan"))),
        "sigmaLPdistr": float(getattr(agg, "std_lp", float("nan"))),
        "extraction_bias": float(extraction_bias),
        "Kolmo_Smirnov": float(
            getattr(agg, "ks_lp_pvalue", float("nan")) or float("nan")
        ),
        "chi2_Kolmo_Smirnov": float(
            getattr(agg, "ks_chi2_pvalue", float("nan")) or float("nan")
        ),
        "LPdistrSave": LPdistrSave,
        "LPdistrErrSave": LPdistrErrSave,
    }

    if extra_fields:
        out_dict.update(extra_fields)
